...
namespace Assets.Scripts.Core.Scene
{
	public class SceneController : MonoBehaviour
	{
		...
		public static int UpperLayerRange;
		...
		{
			GetActiveScene().BackgroundLayer.DrawLayer(texture, 0, 0, 0, null, 0f, /*isBustshot:*/ false, 0, wait, isblocking);
		}
		...
		private void Update()
		{
			if (Screen.width != lastWidth)
			{
				...
				float num3 = 480f;
				float num4 = (num2 / num3 <= num) ? num3 : ((float)Mathf.RoundToInt(num2 / num));
				float num5 = 2f / num4;
		...
		public IEnumerator MODDrawLipSync(int character, int audiolayer, string audiofile)
		{
			...
			Texture2D exp8 = MODSystem.instance.modSceneController.MODLipSyncPrepare_fix(character, "0");
			Texture2D exp7 = MODSystem.instance.modSceneController.MODLipSyncPrepare_fix(character, "1");
			Texture2D exp6 = MODSystem.instance.modSceneController.MODLipSyncPrepare_fix(character, "2");
			...
			if (File.Exists(path))
			{
				...
				for (int j = 0; j < exparray.Length; j++)
				{
					...
					if (exparray[j] == string.Empty)
					{
						exparray[j] = "0";
					}
					if (j > 1 && !exparray[j].Equals(exparray[j - 1]))
					{
						switch (exparray[j])
						{
						case "0":
							MODSystem.instance.modSceneController.MODLipSyncProcess(character, "0", exp8, coroutineId);
							break;
						case "1":
							MODSystem.instance.modSceneController.MODLipSyncProcess(character, "1", exp7, coroutineId);
							break;
						case "2":
							MODSystem.instance.modSceneController.MODLipSyncProcess(character, "2", exp6, coroutineId);
							break;
						}
					}
			...
			else
			{
				...
				{
					count = (int)(GameSystem.Instance.AudioController.GetRemainingVoicePlayTime(audiolayer) * 10f);
				}
				if (count >= 5)
				{
