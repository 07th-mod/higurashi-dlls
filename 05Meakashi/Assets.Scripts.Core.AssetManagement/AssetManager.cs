...
namespace Assets.Scripts.Core.AssetManagement
{
	public class AssetManager
	{
		...
		public string GetAudioFilePath(string filename, Assets.Scripts.Core.Audio.AudioType type)
		{
			...
			string text = null;
			string text2 = Path.Combine(assetPath, archiveNameByAudioType + "/" + filename.ToLower()).Replace("\\", "/");
			string text3 = Path.Combine(assetPath, archiveNameByAudioType + "Alt/" + filename.ToLower()).Replace("\\", "/");
