...
namespace Assets.Scripts.Core.Buriko
{
	public class BurikoScriptFile
	{
		...
		private BurikoVariable OperationSetSpeedOfMessage()
		{
			...
			{
				gameSystem.TextController.OverrideTextSpeed = 50 * (num2 / 100);
			}
			return BurikoVariable.Null;
		}
		...
		private BurikoVariable OperationSelect()
		{
			...
			BurikoString obj = BurikoMemory.Instance.GetMemory(burikoReference.Property) as BurikoString;
			if (obj == null)
			{
				throw new Exception("Unable to read BurikoString type from variable!");
			}
			List<string> stringList = obj.GetStringList();
		...
		private BurikoVariable OperationStartShakingOfWindow()
		{
			...
			float speed = (float)((256 - num) * 5) / 1000f;
		...
		private BurikoVariable OperationDrawScene()
		{
			...
			if (GameSystem.Instance.MessageBoxVisible)
			{
				if (gameSystem.IsSkipping)
				{
					gameSystem.MainUIController.FadeOut(0f, isBlocking: false);
		...
		private BurikoVariable OperationChangeBustshot()
		{
			SetOperationType("ChangeBustshot");
			int layer = ReadVariable().IntValue();
			string textureName = ReadVariable().StringValue();
			float wait = (float)ReadVariable().IntValue() / 1000f;
			bool flag = ReadVariable().BoolValue();
			...
			gameSystem.SceneController.ChangeBustshot(layer, textureName, wait, flag);
			if (flag)
			{
				gameSystem.ExecuteActions();
			}
		...
		private BurikoVariable OperationControlMotionOfSprite()
		{
			...
			BurikoMtnCtrlElement obj = BurikoMemory.Instance.GetMemory(burikoReference.Property) as BurikoMtnCtrlElement;
			if (obj == null)
			{
				throw new Exception("Can't get MtnCtrlElement object with name of " + burikoReference.Property);
			}
			MtnCtrlElement[] allElements = obj.GetAllElements();
		...
		private BurikoVariable OperationSavePoint()
		{
			...
			{
				Debug.LogError("Script operation SavePoint: Cannot create save point while saving is disabled!");
				return BurikoVariable.Null;
			}
			string text = ReadVariable().StringValue();
			string text2 = ReadVariable().StringValue();
		...
		public BurikoVariable ExecuteOperation(BurikoOperations op)
		{
			...
			case BurikoOperations.ChangeBustshot:
				return OperationChangeBustshot();
		...
		public BurikoVariable OperationMODdisableNVLModeInADVMode()
		{
		...
		public BurikoVariable OperationMODenableNVLModeInADVMode()
		{
		...
		public BurikoVariable OperationMODDrawCharacter()
		{
			...
			int num14 = ReadVariable().IntValue();
			int num15 = ReadVariable().IntValue();
			string str = ReadVariable().StringValue();
			string str2 = ReadVariable().StringValue();
			int num13 = ReadVariable().IntValue();
			int num12 = ReadVariable().IntValue();
			int num11 = ReadVariable().IntValue();
			bool move = ReadVariable().BoolValue();
			int oldx = ReadVariable().IntValue();
			int oldy = ReadVariable().IntValue();
			int oldz = ReadVariable().IntValue();
			...
			ReadVariable().IntValue();
			ReadVariable().IntValue();
			int num10 = ReadVariable().IntValue();
			int num9 = ReadVariable().IntValue();
			float wait = (float)ReadVariable().IntValue() / 1000f;
			bool flag = ReadVariable().BoolValue();
			string textureName = str + "0";
			string text = str + str2;
			MODSystem.instance.modSceneController.MODLipSyncInvalidateAndGenerateId(num15);
			...
			{
				MODSystem.instance.modSceneController.MODLipSyncStoreValue(num14, num15, str, num13, num12, num11, num10, num9);
			});
			if (gameSystem.IsSkipping)
			{
				wait = 0f;
			}
			MODSystem.instance.modTextureController.StoreLayerTexture(num14, text);
			gameSystem.SceneController.DrawBustshot(num14, textureName, num13, num12, num11, oldx, oldy, oldz, move, num9, num10, wait, flag);
			if (flag)
			{
				gameSystem.ExecuteActions();
			}
		...
		public BurikoVariable OperationMODDrawCharacterWithFiltering()
		{
			...
			string textureName = str + "0";
			string text = str + str2;
			MODSystem.instance.modSceneController.MODLipSyncInvalidateAndGenerateId(num13);
			if (!MODSystem.instance.modSceneController.MODLipSyncIsEnabled())
			{
				textureName = text;
			}
			...
			{
				MODSystem.instance.modSceneController.MODLipSyncStoreValue(num12, num13, str, num14, num11, num10, 0, num9);
			});
			if (gameSystem.IsSkipping)
			{
				wait = 0f;
			}
			MODSystem.instance.modTextureController.StoreLayerTexture(num12, text);
			gameSystem.SceneController.DrawBustshotWithFiltering(num12, textureName, mask, num14, num11, num10, originx, 0, oldx, oldy, oldz, move, num9, 0, wait, flag);
			if (flag)
			{
				gameSystem.ExecuteActions();
			}
