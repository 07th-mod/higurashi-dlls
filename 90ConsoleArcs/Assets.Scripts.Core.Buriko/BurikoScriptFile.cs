...
namespace Assets.Scripts.Core.Buriko
{
	public class BurikoScriptFile
	{
		...
		private BurikoVariable OperationPlayBGM()
		{
			...
			float num = (float)ReadVariable().IntValue();
			int num2 = ReadVariable().IntValue();
			float volume = num / 128f;
		...
		private BurikoVariable OperationSelect()
		{
			...
			gameSystem.MainUIController.HideLayerBackground(0.25f);
		...
		private void CommandDeclaration()
		{
			...
			string text2 = text;
			if (text2 != null && new Dictionary<string, int>(4)
			{
				{
					"char",
					0
			...
			}.TryGetValue(text2, out int value))
			{
