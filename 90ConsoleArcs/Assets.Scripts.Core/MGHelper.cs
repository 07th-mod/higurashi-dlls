		...
		public static string GetSavePath()
		{
			...
			else
			{
				if (_savepath == string.Empty)
				{
					...
					if (!Directory.Exists(text2) || text2 == string.Empty)
					{
						text2 = Environment.ExpandEnvironmentVariables("%appdata%");
					}
					_savepath = Path.Combine(text2, "Mangagamer\\higurashi04");
				}
				text = _savepath;
			}
			...
			{
				text = Path.Combine(text, saveSubdirectory);
			}
